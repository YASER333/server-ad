...
module.exports = {
  ...
  getMeta: asyncHandler(async (req, res) => {
    ...
    const program_types = await Student.distinct('program_type');
    res.json({ departments: departments.filter(Boolean).sort(), program_types: program_types.filter(Boolean).sort() });
  })
};
