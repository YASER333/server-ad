...
module.exports = {
  parseCsvBuffer,
  parseExcelBuffer,
  // Attendance-specific CSV parser; tolerant header names
  parseAttendanceCsv: (buffer) => {
    const text = buffer.toString('utf-8');
    const records = parse(text, { columns: true, skip_empty_lines: true, trim: true });
    const norm = (s='') => String(s).trim().toLowerCase();
    return records.map((r) => ({
      roll_number: r['roll number'] || r['roll_no'] || r['rollno'] || r['reg no'] || r['regno'] || r['roll'],
      date: r['date'],
      am: r['am'],
      pm: r['pm'],
      training_event: r['training event'] || r['training_event'] || r['event'] || r['event name'],
      training_company: r['training company'] || r['training_company'] || r['company'] || r['company name'],
      remarks: r['remarks']
    }));
  }
};
